...
COLUMNS = ["age", "workclass", "fnlwgt", "education", "education_num", "martial_status", "occupation", "relatinoship", "race", "gender", "capital_gain", "capital_loss", "hours_per_week", "native_counrty", "income_bracket"]

LABEL_COLUMN = "label"
CATEGORICAL_COLUMNS = ["workclass", "education", "martial_status", "occupation", "relationship", "race", "gender", "native_county"]
...
def may_be_download():
    ...
    if FLAGS.train_data:
        ...
        """
        In https://archive.ics.uci.edu/ml/machine-learning-databases/adult/adult.data
...
def build_estimator(model_dir):
    ...
    workclass = tf.contrib.layers.sparse_column_with_hash_bucket("workclass", hash_bucket_size=1000)
    ...
    if FLAGS.model_type == "wide":
        ...
        m = tf.contrib.learn.DNNLinearCombiendClassifier(model_dir=model_dir, linear_feature_columns=wide_columns, dnn_feature_columns=deep_columns, dnn_hidden_units=[100,50])
    return m

def input_fn(df):
    """ Input builder function """
    # Create a dictinary building form each continuous feature column name (k) to 
